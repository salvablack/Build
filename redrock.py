...
# --- MEJORA: Persistencia de columnas nuevas ---
...
if uploaded_file is not None:
    ...
    if current_file_id != st.session_state.last_file_id:
        ...
        st.session_state.df_custom = None # Reset al cargar nuevo archivo
...
with st.spinner("Cargando datos..."):
    # --- MEJORA: Carga inicial vs carga de sesión ---
    if st.session_state.df_custom is None:
        st.session_state.df_custom = load_data(temp_path, file_extension)
    df = st.session_state.df_custom

# --- MEJORA: SECCIÓN PARA CREAR NUEVAS COLUMNAS ---
st.sidebar.subheader("Operaciones entre columnas")
with st.sidebar.expander("➕ Crear columna calculada"):
    new_col_name = st.text_input("Nombre de la nueva columna")
    col_1 = st.selectbox("Columna A", df.columns, key="c1")
    op = st.selectbox("Operación", ["+", "-", "*", "/"], key="op")
    col_2 = st.selectbox("Columna B", df.columns, key="c2")
    if st.button("Crear Columna"):
        if new_col_name:
            try:
                if op == "+": df[new_col_name] = df[col_1] + df[col_2]
                elif op == "-": df[new_col_name] = df[col_1] - df[col_2]
                elif op == "*": df[new_col_name] = df[col_1] * df[col_2]
                elif op == "/": df[new_col_name] = df[col_1] / df[col_2]
...
def footer(canvas, doc):
    ...
    canvas.drawCentredString(A4[1]/2, 1.5*cm, "Generated by RedRock • 2026")
...
def generate_pdf():
    ...
    if grouped_df is not None and not grouped_df.empty:
        ...
        if img_path:
            ...
            elements.append(Image(img_path, width=24*cm, height=12*cm))
            elements.append(PageBreak())
